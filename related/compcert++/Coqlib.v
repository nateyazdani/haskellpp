...
Lemma Z_le_dec_reflexion : forall a b,
  (if Z_le_dec a b then a <= b else True).
Proof.
  intros a b.
  destruct (Z_le_dec a b); intros.
   assumption.
  trivial.
Qed.  

Lemma Z_lt_dec_reflexion : forall a b,
  (if Z_lt_dec a b then a < b else True).
Proof.
  intros a b.
  destruct (Z_lt_dec a b); intros.
   assumption.
  trivial.
Qed.  
...
Lemma option_eq_dec : forall (A : Type) (A_eq_dec : forall a1 a2 : A, {a1 = a2} + {a1 <> a2}),
  forall o1 o2 : option A, {o1 = o2} + {o1 <> o2}.
Proof.
  decide equality.
Qed.

Lemma prod_eq_dec : forall (U : Type) (U_eq_dec : forall u1 u2 : U, {u1 = u2} + {u1 <> u2})
  (V : Type) (V_eq_dec : forall v1 v2 : V, {v1 = v2} + {v1 <> v2}),
  forall p1 p2 : U * V, {p1 = p2} + {p1 <> p2}.
Proof.
  decide equality.
Qed.
