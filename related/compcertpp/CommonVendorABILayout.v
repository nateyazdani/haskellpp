...
Proof.
 ...
 case_eq (hierarchy ! i).
  ...
  intros ; right ; intro Habs ; inversion Habs ; congruence.
...
*)
           ...
           Definition nv_offset :=
            ...
            Proof.
              ...
              case_eq cond.
               unfold cond.
               case em; try congruence.
               case_eq (filter f_empty e); try congruence.
               intros.
               simpl.
               intro; subst.
               generalize (filter_In f_empty (b, co + bo) e).
               rewrite H2.
               simpl.
               destruct 1.
               apply H5.
               split.
               generalize (nvebo_prop Hld He b (co + bo)).
               tauto.
               unfold f_empty.
               ...
               assumption.
              intros _.
              destruct (bounded_offset align_pos (nvl_nvds nvldata) (nvl_nvsize nvldata) f').
              destruct a0.
              destruct H3.
              ...
              destruct H4.
               ...
               omega.
              (* case not away *)
              destruct (is_empty_dec hierarchy_wf ci).
               revert H4.
               unfold f'.
               ...
               assert (
                ...
                destruct (In_dec (prod_eq_dec peq Z_eq_dec) (b, co + bo) (nvl_nvebo' nvldata)); try congruence.
                destruct n.
                generalize (nvl_nvebo'_prop prop b (co + bo)).
                destruct 1.
                apply H19.
                esplit.
                esplit.
                ...
                assumption.
               assumption.
              generalize (nvl_nonempty_high_bound prop n H H13).
              omega.
            Qed.
...
(** Update the data to assign [nv_offset] to the non-virtual base. *)
         ...
         Definition non_virtual_direct_base_layout_step : {
           ...
           exists (
             ...
             (** add non-virtual empty base offsets of [a] to [nvl_nvebo'] if [a] is empty *) ((if em then List.map g e else nil) ++ nvl_nvebo' nvldata)
             (** update data size if [a] is not empty *) (if em then nvl_nvds nvldata else (nv_offset + non_virtual_size o'))
         ...
         Proof.
           ...
           assumption.
            ...
            case em; intro.
             assumption.
            generalize (non_virtual_size_positive (offsets_intro ( Hld) Ho')).
            ...
            case em. 
             ...
             destruct (Z_le_dec (nv_offset + non_virtual_size o') (nvl_nvsize nvldata)).
              eauto.
             eapply Zle_trans with (nvl_nvsize nvldata).
              eauto.
             omega.
            intro.
            destruct (Z_le_dec (nv_offset + non_virtual_size o') (nvl_nvsize nvldata)).
         ...
         split.
          ...
          destruct (in_app_or _ _ _ H).
           ...
           case em.
            ...
            destruct x.
            injection H1; intros; subst.
            destruct (nvl_nvebo'_prop prop i0 z).
            exists a.
            exists nv_offset.            
            rewrite PTree.gss.
            split.
            ...
            split.
             assumption.
            replace (nv_offset + z - nv_offset) with z by omega.
            generalize (nvebo_prop Hld He i0 z).
            tauto.
           simpl; tauto.          
          destruct (nvl_nvebo'_prop prop b o).
          destruct (H1 H0).
          destruct H3.
          exists x.
          exists x0.
...
(** There is a direct non-virtual primary base [a] of [cilimit ld]: choose it as the primary base *)
     ...
     inversion 1.
      ...
      generalize (non_virtual_size_positive  (offsets_intro (Hld) Hof')).
      omega.
      ...
      generalize (non_virtual_data_size_non_virtual_size Hld Hof').
      omega.
 ...
 *)
         ...
         Section prop.
           ...
           Proof.
             intros.
             assert ( assoc (FieldSignature.eq_dec (A := A)) fs1 (fl_offsets fldata) <> None) by congruence.
             destruct (fl_exist prop fs1).              
             assert (In fs1 (Class.fields h)) by eauto.
             assert (Plt ci1 (cilimit ld)) by eauto using Well_formed_hierarchy.well_founded_struct.
             destruct (empty_base_offset_wf 
               hierarchy_wf (offsets_intro ( Hld)) (offsets_guard ( Hld)) H4).
             assert ((offsets ld) ! b <> None) by eauto using Ple_Plt_trans, offsets_exist.
             destruct (empty_base_offset_incl hierarchy_wf (offsets_intro ( Hld)) (offsets_guard ( Hld))
               H4 H19 H3
             ).
             destruct (empty_base_offset_incl hierarchy_wf (offsets_intro (Hld)) (offsets_guard ( Hld))
               H10 H19 H9
             ).
             generalize (fl_non_overlap prop H H5 H11).
             unfold field_size.             
             rewrite H0.
             rewrite H6.
             rewrite H3.
             rewrite H9.
             intros.
             generalize (H24 _ (refl_equal _) _ (refl_equal _)).
             intro.
             generalize (fl_low_bound prop H).
             intros.
             generalize (fl_low_bound prop H5).
             intros.
             generalize (nvl_data_low_bound H_nvldata).
             intros.
             assert (z1 <= Zpos p1 - 1) by omega.
             assert (0 <= size so1) by omega.
             generalize (Zmult_le_compat_r _ _ _ H29 H30).
             intros.
             assert (0 <= size so2) by omega.
             assert (z2 <= Zpos p2 - 1) by omega.
             generalize (Zmult_le_compat_r _ _ _ H33 H32).
             intros.
             generalize (Zmult_ge H1 H30).
             generalize (Zmult_ge H7 H32).
             omega.
           Qed.
...
*)
             ...
             Proof.
               destruct (bounded_offset (align_low_bound (offsets_intro ( Hld) Ho')) (fl_nvds fldata) (fl_nvsize fldata) (fun z => match List.filter (f z) e' with nil => true | _ => false end)).
               destruct a0.
               destruct H0.
               esplit.
               split.
            ...
            Proof.
              ...
              destruct (is_empty_dec hierarchy_wf ci).
               ...
               intros.
               intro Habs.
               destruct (filter_In (f off) (b, i * size o' + bo') e').
               rewrite H21 in H26.
               simpl in H26.
               apply H26.
               split.
               ...
               intros.
               destruct n.
               destruct (nvl_nvebo'_prop H_nvldata b (co + bo)).
               apply H28.
               esplit.
               esplit.
               split.
              ...
              assumption.
             (* case not empty *)
             generalize (nvl_nonempty_high_bound H_nvldata n H H16).
           ...
           exists (
             ...
             (** update the data size *) (fl_offset + Zpos na * size o')
...
(** [nvebo0] is the set of all non-virtual empty base offsets, except the offset of [cilimit ld] itself. *)
  ...
  Proof.
    ...
    destruct (in_app_or _ _ _ H).
     ...
     omega.
    (* field *)
    cut (0 <= o < fl_nvds fldata).
     generalize (fl_data_size H_fldata).
     omega.
  ...
  Proof.
    ...
    destruct (in_app_or _ _ _ H).
     ...
     intros.
     destruct (non_virtual_empty_base_offset_wf 
       hierarchy_wf (offsets_intro ( Hld)) (offsets_guard ( Hld)) H5).
     assert (Plt b (cilimit ld)) by eauto using Ple_Plt_trans.
     assert ((offsets ld) ! b <> None) by eauto using offsets_exist.
     destruct (non_virtual_empty_base_offset_incl hierarchy_wf
       (offsets_intro ( Hld)) (offsets_guard ( Hld))
       H5 H17 H13).
     destruct (is_empty_dec hierarchy_wf x).
      destruct H0.
      destruct (nvl_nvebo'_prop H_nvldata b o).
      apply H20.
      esplit.
      esplit.
      split.
      ...
      assumption.
      assumption.
     generalize (nvl_low_bound H_nvldata H4).     
     generalize (nvl_nonempty_high_bound H_nvldata n H4 H13).
     omega.
    (* field *)
    clear H0.
    rename H1 into H0.
...
- [vl_nvebo']: the set of non-virtual empty base offsets reachable through an *empty* virtual base that has already been assigned an offset
           ...
           Definition v_offset :=
            ...
            Proof.
              ...
              case_eq cond.
               ...
               tauto.
               unfold f_empty.
               case (In_dec (prod_eq_dec peq Z_eq_dec) (b, co + bo) (vl_nvebo vldata)).
                destruct (In_dec (prod_eq_dec peq Z_eq_dec) (b, co + bo) nvebo0); trivial.
               intros.
               destruct n.
               destruct (vl_nvebo_prop prop b (co + bo)).
               apply H7.
               esplit.
               esplit.
               split.
               ...
               assumption.
              intros _.
              destruct (bounded_offset align_pos (vl_ds vldata) (vl_size vldata) f').
              destruct a0.
              destruct H3.
              assert (Plt ci (cilimit ld)).
              ...
              destruct (is_empty_dec hierarchy_wf ci).
               ...
               assert (
                ...
                destruct (In_dec (prod_eq_dec peq Z_eq_dec) (b, co + bo) (vl_nvebo' vldata)).
                 destruct ( In_dec (prod_eq_dec peq Z_eq_dec) (b, co + bo) (nvl_nvebo' nvldata)); trivial.
                destruct n.
                generalize (vl_nvebo'_prop prop b (co + bo)).
                destruct 1.
                apply H19.
                esplit.
                esplit.
                ...
                assumption.
               assumption.
              generalize (vl_nonempty_high_bound prop n H H13).
              omega.
            Qed.
            ...
            Proof.
              ...
              case_eq cond.
               ...
               case_eq (filter f_empty e); try congruence.
               intros.
               generalize (filter_In f_empty (b, bo') e).
               rewrite H4.
               simpl.
               destruct 1.
               apply H8.
               ...
               tauto.
               unfold f_empty.
               case (In_dec (prod_eq_dec peq Z_eq_dec) (b, bo') nvebo0).
                trivial.
               intros; contradiction.
              intros _.
              destruct (bounded_offset align_pos (vl_ds vldata) (vl_size vldata)).
              destruct a0.
              destruct H5.
              intro.
              destruct H6.
             ...
             split.
              ...
              tauto.
             assert (
   (if In_dec (prod_eq_dec peq Z_eq_dec) (b, x + bo') (nvl_nvebo' nvldata)
    then true
    else
     ...
     then true
             ...
             case ( In_dec (prod_eq_dec peq Z_eq_dec) (b, x + bo') (nvl_nvebo' nvldata)).
              trivial.
             intros.
             apply False_rect.
             generalize (nvebo0_data_incl H7 n).
         ...
         Definition virtual_base_layout_step : {
           ...
           exists (
             ...
             (** add the non-virtual empty base offsets of [a] to the empty base offsets of [cilimit ld] reachable through an empty virtual base, if [a] is empty *) ((if em then List.map g e else nil) ++ vl_nvebo' vldata)
             (** update the data size if [a] is not empty *) (if em then vl_ds vldata else (v_offset + non_virtual_size o'))
         ...
         Proof.
           ...
           assumption.
            ...
            case em; intro.
             apply Zle_trans with (vl_size vldata).
              assumption.
            destruct (Z_le_dec (v_offset + non_virtual_size o') (vl_size vldata)); omega.
            destruct (Z_le_dec (v_offset + non_virtual_size o') (vl_size vldata)); omega.
        ...
        assumption.
         ...
         destruct (in_app_or _ _ _ H).
          ...
          destruct x.
          injection H1; intros; subst.
          destruct (nvebo_prop ( Hld) He i0 z).
          exists a.
          exists v_offset.
          rewrite PTree.gss.
          split.
...
(** The proof that the new offsets computed for the current class satisfy the soundness conditions *)
  ...
  Proof.
    ...
    intros.
    generalize (nvl_nonempty_non_overlap H_nvldata H H0 H1 H2 H3 H4 H5).
    generalize (non_virtual_data_size_non_virtual_size Hld H4).
    generalize (non_virtual_data_size_non_virtual_size Hld H5).
    omega.

    intros until 2.
    rewrite (nvl_offsets_offsets' H0).
    intros.
    generalize (nvl_nonempty_high_bound H_nvldata H H0 H1).
    generalize (non_virtual_data_size_non_virtual_size Hld H1).
    omega.    
    ...
    intros; eauto using fl_low_bound.
    ...
    intros until 5.
    generalize (fl_non_overlap H_fldata H H0 H3).
    unfold field_size, field_data_size.
    case_eq (FieldSignature.type f1).
     case_eq (FieldSignature.type f2).
      ...
      case_eq ((offsets ld) ! struct); try congruence.
       intros.
       generalize (data_size_high_bound (offsets_intro ( Hld) H5)).
       generalize (virtual_base_offsets_data_size (offsets_intro Hld H5) (virtual_base_offsets_self (offsets_intro Hld H5))(H2 _ _ H4) H5).
       generalize (non_virtual_data_size_own_fields_offset (offsets_intro Hld H5)).
       generalize (own_fields_offset_low_bound (offsets_intro Hld H5)).
       generalize (H7 _ (refl_equal _) _ (refl_equal _)).
       injection H8; intro; subst.
       Opaque Zminus. injection H9; intro; subst.
       Transparent Zminus. omega.
     intros until 1.
   ...
   generalize (data_size_high_bound (offsets_intro ( Hld) H5)).
   intro.
       generalize (virtual_base_offsets_data_size (offsets_intro Hld H5) (virtual_base_offsets_self (offsets_intro Hld H5))(H1 _ _ H4) H5).
       generalize (non_virtual_data_size_own_fields_offset (offsets_intro Hld H5)).
       generalize (own_fields_offset_low_bound (offsets_intro Hld H5)).
  intros until 3.
  case_eq (FieldSignature.type f2).
   Opaque Zminus. injection 3; intro; subst.
   injection 1; intro; subst.
   generalize (H11 _ (refl_equal _) _ (refl_equal _)).
   omega.
  intros until 1.
  rewrite (fl_offsets_offsets' H0 H10).
  case_eq ((offsets ld) ! struct0); try congruence.
  intros until 1.
   generalize (data_size_high_bound (offsets_intro ( Hld) H11)).
   intro.
       generalize (virtual_base_offsets_data_size (offsets_intro Hld H11) (virtual_base_offsets_self (offsets_intro Hld H11))(H2 _ _ H10) H11).
       generalize (non_virtual_data_size_own_fields_offset (offsets_intro Hld H11)).
       generalize (own_fields_offset_low_bound (offsets_intro Hld H11)).
  injection 5; intro; subst.
  injection 1; intro; subst.
  generalize (H16 _ (refl_equal _) _ (refl_equal _)).
  omega.
  ...
  case_eq (Class.fields h).
   ...
   generalize (fl_data_low_bound H_fldata).
   generalize (non_virtual_size_positive (offsets_intro Hld H7)).
   omega.
   intros.
   ...
   simpl.
   destruct 1.
   destruct (H1 (or_introl _ (refl_equal _))); try contradiction. 
   case_eq (assoc (FieldSignature.eq_dec (A := A)) t (fl_offsets fldata)); try congruence.
   intros.
   generalize (fl_low_bound H_fldata H4).
   generalize (nvl_data_low_bound H_nvldata).
   generalize (fl_high_bound H_fldata H4).
   unfold field_size.
   case_eq (FieldSignature.type t).
    intros.
    generalize (H6 _ (refl_equal _)).
    generalize (typ_size_positive PF ( t0)).
    omega.
   intros until 1.
   case_eq ((offsets ld) ! struct); try congruence.
   intros.
   generalize (H7 _ (refl_equal _)).
   ...
   generalize (Zmult_ge H13 H14).
   omega.
   intros.
   assert (In t (Class.fields h)).
     rewrite H0; auto.
   ...
   unfold field_data_size, field_size.
   case_eq (FieldSignature.type f).
    injection 3; intro; subst.
    generalize (H1 _ (refl_equal _)).
   tauto.
   intros until 1.
   rewrite (fl_offsets_offsets' H H0).
   case_eq ((offsets ld) ! struct); try congruence.
   Opaque Zminus. injection 3; intros; subst.
   generalize (H2 _ (refl_equal _)).
   generalize (data_size_high_bound (offsets_intro ( Hld) H1)).
   omega.
  ...
  omega.
   ...
   eauto using fl_data_low_bound, Zle_trans, fl_size_low_bound.
   ...
   intros.
   generalize (non_virtual_data_size_non_virtual_size Hld H0).
   generalize (non_virtual_data_size_non_virtual_size Hld H2).
   generalize (vl_nonempty_non_overlap H_vldata H H3 H1 H4 H5 H0 H2).
   omega.
   assumption.
   assumption.
  ...
  rewrite PTree.gso.
  intros. 
  generalize (vl_nonempty_high_bound H_vldata H0 H H1).
  generalize (non_virtual_data_size_non_virtual_size Hld H1).
  omega.
  assumption.
  assumption.
...
Proof.
  ...
  assert (Plt ci2 (cilimit ld)).
   ...
   congruence.
   eassumption.
  eapply fl_disjoint_field_empty_bases.
  eauto.
  eassumption.
  eassumption.
  assumption.
...
Proof.
  constructor; simpl.
   ...
   destruct (peq ci (cilimit ld)).
    ...
    simpl.
    eauto using fl_data_size.
   rewrite PTree.gso; eauto using non_virtual_data_size_non_virtual_size.
