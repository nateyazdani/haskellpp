...
Lemma goal :
  forall obj ar i h P tinit init body vars k2 current other sf, sf = (StackFrame.Kconstrother obj ar i (h, P) tinit init body vars Constructor.base k2 current other) -> forall (Hin: In sf (State.stack s2)), stackframe_weak_inv prog s2 sf.
Proof.
 inversion Hs1.
...
destruct Hin as [Heq | Hin2].
 injection Heq. 
 intros; subst.
 generalize (StackFrame.Kconstrother_base_inj Heq).
 intro; invall; subst.
 revert kind.
 unfold state_kind_inv.
 simpl.
...
pointer_eq_dec (A:=A)
 ...
 destruct k2; simpl; intro; invall; subst; unfold_ident_in_all; abstract congruence.
destruct k0. 
 sdestruct (
pointer_eq_dec (A:=A)
                        (obj,
 ...
 destruct k2.
  ...
  destruct (H7 _ _ _ _ (refl_equal _)).
...
intro; destruct k2.
 ...
 revert kind.
 unfold state_kind_inv; simpl.
 intro; invall; subst; unfold_ident_in_all; destruct H14; abstract congruence.
sdestruct (
pointer_eq_dec (A:=A) (obj, (ar, i, (h, p)))
                        (obj0,
                        (ar0, i0, (Class.Inheritance.Shared, current :: nil)))
...
revert kind.
unfold state_kind_inv; simpl.
intro; invall; subst; unfold_ident_in_all; destruct H16; abstract congruence.
...
pointer_eq_dec (A:=A)
 ...
 unfold state_kind_inv; simpl.
 injection e;
 invall; intros; destruct beta; invall; subst.
  generalize (last_complete p b).
  rewrite H8.
  rewrite last_complete.
  injection 1; intro; subst.
  generalize (app_reg_r H8).
  intro; unfold_ident_in_all; abstract congruence.
 destruct P.
  discriminate.
 destruct P; discriminate.
...
invall; subst.
generalize (H21 _ (or_introl _ (refl_equal _))).
